...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedSamsatCars', function(source, cb)
	local ownedSamsatCars = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if Config.Main.ShowVehLoc then
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
			...
			for _,v in pairs(data) do
				local vehicle = json.decode(v.vehicle)
				table.insert(ownedSamsatCars, {vehicle = vehicle, stored = v.stored, plate = v.plate})
			end
			cb(ownedSamsatCars)
		end)
	else
...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedSamsatAircrafts', function(source, cb)
	local ownedSamsatAircrafts = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if Config.Main.AdvVehShop then
		if Config.Main.ShowVehLoc then
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
				...
				for _,v in pairs(data) do
					local vehicle = json.decode(v.vehicle)
					table.insert(ownedSamsatAircrafts, {vehicle = vehicle, stored = v.stored, plate = v.plate, vtype = 'aircraft'})
				end
				cb(ownedSamsatAircrafts)
			end)
		else
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored', { -- job = NULL
				['@owner'] = xPlayer.identifier,
		...
		if Config.Main.ShowVehLoc then
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
				...
				for _,v in pairs(data) do
					local vehicle = json.decode(v.vehicle)
					table.insert(ownedSamsatAircrafts, {vehicle = vehicle, stored = v.stored, plate = v.plate, vtype = 'helicopter'})
				end
				cb(ownedSamsatAircrafts)
			end)
		else
...
ESX.RegisterServerCallback('esx_advancedgarage:getOutOwnedSamsatCars', function(source, cb)
	local ownedSamsatCars = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND job = @job AND `stored` = @stored', {
		...
		for _,v in pairs(data) do
			local vehicle = json.decode(v.vehicle)
			table.insert(ownedSamsatCars, vehicle)
		end
		cb(ownedSamsatCars)
	end)
end)

ESX.RegisterServerCallback('esx_advancedgarage:checkMoneySamsat', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getMoney() >= Config.Samsat.PoundP then
		cb(true)
	else
		cb(false)
	end
...
RegisterServerEvent('esx_advancedgarage:paySamsat')
AddEventHandler('esx_advancedgarage:paySamsat', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeMoney(Config.Samsat.PoundP)
	TriggerClientEvent('esx:showNotification', source, _U('you_paid') .. Config.Samsat.PoundP)

	if Config.Main.GiveSocMoney then
		TriggerEvent('esx_addonaccount:getSharedAccount', 'society_mechanic', function(account)
			account.addMoney(Config.Samsat.PoundP)
		end)
	end
end)
-- End of Samsat Code
