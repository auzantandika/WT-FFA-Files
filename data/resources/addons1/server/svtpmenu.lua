	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			if Config.EnableJobLogs == true then
				TriggerEvent('esx_ttyy:AddLog', 'police', 'putStockItems', count, inventoryItem.label)
			end
...
function sendToDiscord (discordnya, name, message, color)
	local DiscordWebHook = discordnya
	local embeds = {
		{
			["title"]= message,
			["type"]= "rich",
...
function loadLogs()
	LogPolice      = LoadResourceFile("esx_policejob", "Logs/police.log")      or ""
end

function SaveInLog(job, message)
	if job == "police" then
        LogPolice = LogPolice .. message .. "\n"
        SaveResourceFile("esx_policejob", "Logs/police.log", LogPolice, -1)
...
RegisterServerEvent('esx_ttyy:AddLog')
AddEventHandler('esx_ttyy:AddLog', function(job, localetxt, info1, info2, info3, info4)
  local _job        = job
