...
function UpdatePlayerTable(connectedPlayers)
	...
	local ems, police, cardealer, mechanic, samsat, pedagang, players = 0, 0, 0, 0, 0, 0, 0

	for k,v in pairs(connectedPlayers) do

		if num == 1 then
			table.insert(formattedPlayerList, ('<tr><td>%s</td><td>%s</td><td>%s</td>'):format(v.identifier, v.id, v.ping))
			num = 2
		elseif num == 2 then
			table.insert(formattedPlayerList, ('<td>%s</td><td>%s</td><td>%s</td></tr>'):format(v.identifier, v.id, v.ping))
			num = 1
		end
	...
	SendNUIMessage({
		...
		jobs   = {ems = ems, police = police, cardealer = cardealer, mechanic = mechanic, samsat = samsat, pedagang = pedagang, player_count = players}
	})
end
