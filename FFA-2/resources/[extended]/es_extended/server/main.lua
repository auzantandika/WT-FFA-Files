...
AddEventHandler('chatMessage', function(playerId, author, message)
	if message:sub(1, 1) == '/' and playerId > 0 then
        ...
        --     template = '<div class="chat-message server"><b>SYSTEM:</b> Command Not Found / Invalid Usage.</div>',
        --     args = { message }
        -- })
	end
end)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = _U('gave_item', itemCount, sourceItem.label, target) })
				TriggerClientEvent('mythic_notify:client:SendAlert', target, { type = 'inform', text = _U('received_item', itemCount, sourceItem.label, source) })
			else
				-- sourceXPlayer.showNotification(_U('ex_inv_lim', targetXPlayer.name))
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = _U('ex_inv_lim', targetXPlayer.name) })
				
			end
		else
			-- sourceXPlayer.showNotification(_U('imp_invalid_quantity'))
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = _U('imp_invalid_quantity') })
		end
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			if itemName == "money" then 
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = _U('gave_account_money', ESX.Math.GroupDigits(itemCount), target) })
				TriggerClientEvent('mythic_notify:client:SendAlert', target, { type = 'inform', text = _U('received_account_money', ESX.Math.GroupDigits(itemCount), source) })
			elseif itemName == "black_money" then 
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = _U('gave_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName], target) })
				TriggerClientEvent('mythic_notify:client:SendAlert', target, { type = 'inform', text = _U('received_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName], source) })
			end
		else
			-- sourceXPlayer.showNotification(_U('imp_invalid_amount'))
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = _U('imp_invalid_amount') })
		end
		...
		if sourceXPlayer.hasWeapon(itemName) then
			...
			if not targetXPlayer.hasWeapon(itemName) then
				...
				if weaponObject.ammo and itemCount > 0 then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = _U('gave_weapon_withammo', weaponLabel, itemCount, ammoLabel, target) })
					TriggerClientEvent('mythic_notify:client:SendAlert', target, { type = 'inform', text = _U('received_weapon_withammo', weaponLabel, itemCount, ammoLabel, source) })
				else
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = _U('gave_weapon', weaponLabel, targetXPlayer.name) })
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = _U('gave_weapon_hasalready', targetXPlayer.name, weaponLabel) })
				TriggerClientEvent('mythic_notify:client:SendAlert', target, { type = 'inform', text = _U('received_weapon_hasalready', sourceXPlayer.name, weaponLabel) })
			end
		end
...
AddEventHandler('esx:useItem', function(itemName)
	...
	if count > 0 then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = _U('act_imp') })
	end
end)
